...
<div class="max-w-4xl mx-auto space-y-8">
    <div class="flex items-center space-x-4">
        ...
        <h3 class="text-3xl font-black text-slate-800">Tulis Artikel Baru</h3>
    </div>

    <form action="{{ route('depootcom.admin.blog.store') }}" method="POST" enctype="multipart/form-data" class="space-y-6">
        @csrf
        <div class="bg-white p-8 rounded-[2.5rem] shadow-sm border border-slate-100 space-y-6">
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div class="space-y-2">
                    ...
                    <input type="text" name="title" required placeholder="Masukkan judul yang menarik..." class="w-full bg-slate-50 border border-slate-200 rounded-2xl px-6 py-4 focus:outline-none focus:border-blue-500 transition-colors font-bold text-lg">
                </div>
                <div class="space-y-2">
            ...
            <div class="space-y-2">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6 items-center">
                    <div class="relative group h-40 bg-slate-50 border-2 border-dashed border-slate-200 rounded-2xl flex flex-col items-center justify-center overflow-hidden transition-all hover:border-blue-400">
                        ...
                        <img id="preview-placeholder" src="" class="absolute inset-0 w-full h-full object-cover hidden">
                        <div id="upload-hint" class="flex flex-col items-center text-slate-400 group-hover:text-blue-500 transition-colors">
                            <i class="fas fa-cloud-upload-alt text-3xl mb-2"></i>
                            <span class="text-[10px] font-black uppercase tracking-widest">Pilih Gambar Utama</span>
                        </div>
                    </div>
                    <div class="text-[10px] text-slate-400 font-medium italic">
                        *Gambar ini akan ditampilkan di bagian atas artikel dan list blog. Gunakan format JPG/PNG, ukuran maksimal 2MB.
                    </div>
                </div>
            </div>
            ...
            <div class="space-y-2">
                ...
                <textarea name="excerpt" rows="2" placeholder="Tulis ringkasan singkat artikel..." class="w-full bg-slate-50 border border-slate-200 rounded-2xl px-6 py-4 focus:outline-none focus:border-blue-500 transition-colors font-medium"></textarea>
            </div>
            ...
            <div class="space-y-4">
                ...
                <textarea name="content" id="content-textarea" class="hidden" required></textarea>
            ...
            <div class="flex items-center justify-between p-6 bg-slate-50 rounded-[2.5rem] border border-slate-100">
                <div class="flex items-center space-x-4">
                    ...
                    <div>
                        ...
                        <p class="text-[10px] text-slate-400 font-medium">Aktifkan untuk langsung menampilkan artikel di publik.</p>
                    </div>
                </div>
...
<script>
    document.addEventListener('DOMContentLoaded', () => {
        const tiptap = window.setupTiptap('#tiptap-editor', '#content-textarea', '{{ route('depootcom.admin.upload-image') }}', '{{ csrf_token() }}');
        ...
        btnAi.addEventListener('click', async () => {
            ...
            if (!confirm(`Hasilkan konten AI untuk topik: "${topic}"? Ini mungkin memakan waktu beberapa menit.`)) return;
            ...
            try {
                ...
                if (data.error) {
                    ...
                    // Gunakan HTML yang sudah dikonversi dari backend Laravel!
                    if (window.editor) {
