...
<div class="max-w-2xl mx-auto space-y-8">
    <div class="flex items-center space-x-4">
        ...
        <h3 class="text-3xl font-black text-slate-800">Kategori Baru</h3>
    </div>

    <form action="{{ route('depootcom.admin.categories.store') }}" method="POST" class="space-y-6">
        @csrf
        <div class="bg-white p-8 rounded-[2.5rem] shadow-sm border border-slate-100 space-y-6">
            <div class="space-y-2">
                ...
                <input type="text" name="name" required placeholder="Contoh: Networking, Software..." class="w-full bg-slate-50 border border-slate-200 rounded-2xl px-6 py-4 focus:outline-none focus:border-blue-500 transition-colors font-bold text-lg">
            </div>

            <div class="space-y-2">
                <label class="text-xs font-black text-slate-400 uppercase tracking-widest ml-2">Deskripsi (Optional)</label>
                <textarea name="description" rows="4" placeholder="Penjelasan singkat tentang kategori ini..." class="w-full bg-slate-50 border border-slate-200 rounded-2xl px-6 py-4 focus:outline-none focus:border-blue-500 transition-colors font-medium"></textarea>
            </div>
        </div>
