...
<div class="max-w-7xl mx-auto" x-data="{ showModal: false, formData: { id: '', description: '', amount: '', date: '{{ date('Y-m-d') }}' } }">
    ...
    <div class="bg-white p-4 rounded-xl shadow-sm border border-slate-200 mb-6 flex flex-col xl:flex-row gap-6 justify-between items-start xl:items-end">
        <!-- Date Filter Form -->
        <form action="{{ route('billing.debt') }}" method="GET" class="flex flex-col sm:flex-row gap-4 items-end w-full xl:w-auto flex-grow">
            ...
            <div class="flex flex-col sm:flex-row gap-4 w-full sm:w-auto">
                <div class="w-full sm:w-40 md:w-48">
                    ...
                    <input type="date" name="start_date" value="{{ $startDate ?? '' }}" class="w-full bg-slate-50 border-slate-200 rounded-lg text-sm focus:ring-yellow-500 focus:border-yellow-500">
                </div>
                <div class="w-full sm:w-40 md:w-48">
                    <label class="block text-xs font-bold text-slate-500 mb-1">SAMPAI TANGGAL</label>
                    <input type="date" name="end_date" value="{{ $endDate ?? '' }}" class="w-full bg-slate-50 border-slate-200 rounded-lg text-sm focus:ring-yellow-500 focus:border-yellow-500">
                </div>
            </div>
            <div class="flex flex-wrap gap-2 w-full sm:w-auto">
                ...
                <a href="{{ route('billing.debt.export', ['start_date' => $startDate, 'end_date' => $endDate, 'search' => $search ?? '']) }}" class="flex-1 sm:flex-none justify-center bg-green-600 text-white px-4 py-2 rounded-lg text-sm font-bold hover:bg-green-700 transition shadow-sm whitespace-nowrap flex items-center">
                    <i class="fas fa-file-excel mr-2"></i> Export
                </a>
                @endif
                @if(!empty($startDate) || !empty($endDate))
                <a href="{{ route('billing.debt', ['search' => $search ?? '']) }}" class="flex-1 sm:flex-none justify-center bg-slate-100 text-slate-600 px-4 py-2 rounded-lg text-sm font-bold hover:bg-slate-200 transition flex items-center">
                    Reset
                </a>
                @endif
            </div>
        ...
        <!-- Search Form -->
        <form action="{{ route('billing.debt') }}" method="GET" class="flex gap-2 items-end w-full xl:w-auto pt-2 border-t border-slate-100 xl:border-t-0 xl:pt-0">
            ...
            <div class="w-full sm:w-64 flex-grow">
                ...
                <input type="text" name="search" value="{{ $search ?? '' }}" placeholder="Ketik kata kunci..." class="w-full bg-slate-50 border-slate-200 rounded-lg text-sm focus:ring-yellow-500 focus:border-yellow-500">
            </div>
            <div class="flex gap-2 shrink-0">
                ...
                @if(!empty($search))
                <a href="{{ route('billing.debt', ['start_date' => $startDate ?? '', 'end_date' => $endDate ?? '']) }}" class="bg-slate-100 text-slate-600 px-3 py-2 rounded-lg text-sm font-bold hover:bg-slate-200 transition flex items-center justify-center h-[38px] w-[46px]" title="Hapus Pencarian">
